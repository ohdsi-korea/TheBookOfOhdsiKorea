...
\section*{한국판에 부쳐}\label{-}
\addcontentsline{toc}{section}{한국판에 부쳐}
...
\begin{verbatim}
## [1] "SELECT * FROM temp_schema.y6hnhb7wchildren ;"
\end{verbatim}
...
(``Myocardial infarction'') 과 그 모든 하위 개념들을 포함했고,
\href{http://athena.ohdsi.org/search-terms/terms/314666}{314666} (``Old
myocardial infarction'') 과 그 모든 하위 개념들은 제외했다.
...
과 그 모든 하위 개념들을 포함할 것이고, 하위 개념들 중에서 ``Old
myocardial infarction'' 와 그 모든 하위 개념들은 제외할 것이다.
결과적으로 거의 100개 정도의 표준 개념들을 포함한 개념 모음이
만들어졌다. 이 표준 개념들은 다양한 데이터베이스에서 사용되는 수백 개의
...
통합되어 있다. \index{data quality!data quality check} \index{ACHILLES}
