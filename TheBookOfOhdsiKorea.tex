...
\addcontentsline{toc}{section}{The Book of OHDSI가 쓰인 과정}
...
\item
  ...
  구현할 수 있다.\index{Common Data Model!technology neutrality}
\item
...
다르다 (그림 \ref{fig:domains} 참조). \index{domain!concept}
...
\addcontentsline{toc}{subsubsection}{원천 용어의 OMOP Concept으로의 매핑
검토}
...
단계이다. \index{ETL!quality control}
...
기록에서 나타나는 4가지 문제를 발견하라 (공간상 축약된 형태의 표):
...
같이 있다: PATIENT, START, END에 따라 오름차순으로 데이터를 정렬하라.
그다음 PERSON\_ID 별로, 하나의 기록의 END 시간과 다음 기록의 START
시간의 차이가 1일 이하인 기록을 하나로 만들어 준다. 각 통합된 입원 환자
기록은 하나의 입원 환자 방문으로 간주하며, 다음과 같이 설정한다:
...
입력한다: \index{SQL} \index{structured query language|see {SQL}}
...
\begin{verbatim}
## [1] "SELECT * FROM temp_schema.x3l2v6hpchildren ;"
\end{verbatim}
...
제외되는지를 구체적으로 설명한다. \index{code set}
...
하나의 코호트에 여러 번 속할 수 있다.\index{cohort!exit criteria}
...
\addcontentsline{toc}{subsubsection}{시나리오 1: 당신은 아직 concept
set을 만들지 않았다}
...
\addcontentsline{toc}{subsubsection}{시나리오 2: 당신은 이미 concept
set을 만들었다}
...
반복하여 실행하였다. \index{characterization!treatment pathways}
...
시간의 틀 안에서 발생한 outcome의 비율이다. \index{incidence!proportion}
...
\index{propensity model} \index{propensity score!matching}
...
트리밍을 하는 것이 바람직할 수 있다. \index{propensity score!trimming}
...
척도 caliper scales}로 정의할 수 있다: \index{caliper!scale}
...
같이 정의한다: \index{target cohort} \index{outcome cohort}
\index{time-at-risk}
...
사용한다. \index{supervised learning} 따라서, 대상 코호트에 있는 사람의
...
하이퍼-파라미터 hyper-parameters가 종종 있다. \index{decision boundary}
...
사용한다. \index{LASSO} \index{logistic regression}
...
수 있다. \index{variance} \index{hyper-parameter}
...
Boosting을 사용한다. \index{gradient boosting} \index{xgboost}
...
Python에 있는 sklearn AdaboostClassifier 을 사용한다. \index{adaboost}
\index{python}
...
가중치를 업데이트한다. \index{neural network} \index{perceptron}
...
것이다.\index{angioedema} \index{ACE inhibitors}
...
\addcontentsline{toc}{subsubsection}{모델 설정 Model Setting}
...
한다. \index{evidence quality} \index{regulatory decision-making}
...
정의의 타당성을 고려해야 한다. \index{community}
...
있다. \index{data quality!data quality check} \index{ACHILLES}
...
정의가 어떻게 표현되는지 보여준다. \index{xSpec cohort}
...
\addcontentsline{toc}{subsubsection}{두 번째 단계 : xSens 코호트 정의}
...
xSens 코호트를 ATLAS에서 어떻게 만드는지 보여준다. \index{xSens cohort}
...
\addcontentsline{toc}{subsubsection}{세 번째 단계 : 예측 모델에 적합}
...
\addcontentsline{toc}{subsubsection}{네 번째 단계 : 평가용 코호트 생성}
...
(``\ldots{}5XMI\_eval\ldots{}rds'') 이 모델 파일을 사용해야 한다고
명시한다 (``\ldots{}5XMI\_train\ldots{}rds''). 이 과정에서 생성된 모델과
평가 코호트 파일은 다음 과정에서 제공될 코호트 정의의 평가에 사용될
것이다.

\subsubsection*{다섯 번째 단계 : 코호트 정의 생성 및
검증}\label{-------}
\addcontentsline{toc}{subsubsection}{다섯 번째 단계 : 코호트 정의 생성
및 검증}
...
수 있다는 장점이 있다. \index{software validity}
...
중요한 원칙을 강조하도록 하겠다. \index{programming best practices}
...
홈페이지 ``\url{http://seoji.nl.go.kr}'' 와 국가자료종합목록 구축시스템
``\url{http://kolis-net.nl.go.kr}'' 에서 이용하실 수 있습니다.
