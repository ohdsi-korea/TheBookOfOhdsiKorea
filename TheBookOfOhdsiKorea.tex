...
\item
  ...
  \item
    ...
    concepts을 통합한 날을 알지 못하거나, 알려지지 않은 경우 1970-1-1.
  \item
    VALID\_END\_DATE: 사용 중단을 나타내는 과거의 날 또는 해당 용어가
    알려지지 않은 경우 concept이 누락되거나 비활성화 된 vocabulary의
    갱신일이 알려지지 않은 경우.
  \item
    INVALID\_REASON: ``D''
  \end{itemize}
\item
  ...
  \item
    ...
    concepts을 통합한 날을 알지 못하거나, 알려지지 않은 경우 1970-1-1.
  \item
    VALID\_END\_DATE: 과거에 업그레이드를 나타내는 날 또는 업그레이드가
    포함 된 vocabulary의 갱신일을 알 수 없는 경우.
  \item
    INVALID\_REASON: ``U''
  \end{itemize}
\item
...
가능하다. 비활성 관계는 내부처리만을 위한 Palls 시스템에서 유지된다.
RELATIONSHIP 테이블은 전체 relationship IDs 목록 및 그 반대의 목록과
함께 참조로 사용된다.
...
잃게 된다. Up-hill 매핑은 정보가 유실 되도 표준 연구 use case 들과 관련
없다고 간주될 때에만 사용해야 한다.
...
있다. 이들 중 다수는 clinical events 사이의 관계를 정의하고 정보 검색에
이용될 수 있다. 예를 들어, disorders of the urethra는``finding site of''
관계에 따라 찾을 수 있다. (표\ref{tab:findingSite}참조)
...
일반적으로, SNOMED와 같이 표준 개념을 도출하는데, 사용되는
vocabularies는 더 나은 구조화를 위해 선택되며, 따라서 더 높은 품질의
내부 관계를 갖는 경향이 있다.
...
이것들은 미국 정부가 이것을 어떻게 정의하는지에 대한 정의를 따른다.
민족은 Hispanic 또는 non-Hispanic 인구를 구분하는데, 인종은 상관 없다.
...
\item
  모든 개념들은 한 개의 도메인에 선정되는데, 개념에 의해 표현되는 사건이
  CDM 의 저장되는 위치를 관리한다.
\item
  다른 vocabularies에서 동등한 의미를 갖는 개념들은 그 중 하나에
  매칭되는데, 이는 표준 개념을 가리킨다. 다른 개념들은 소스 개념들이다.
\item
...
\begin{verbatim}
## [1] "SELECT * FROM temp_schema.hsuuex5dchildren ;"
\end{verbatim}
