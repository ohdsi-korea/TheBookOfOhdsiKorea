...
<p class="caption">(\#fig:shinyCal)모델 보정</p>
</div>
...
<p class="caption">(\#fig:shinyAtt)분석한 예측 문제에서의 손실 도표</p>
</div>
...
<p class="caption">(\#fig:multiShinySummary)학습된 각 모델에 대해서 핵심 성능 지표를 보여주는 shiny 요약 페이지</p>
</div>
...
<p class="caption">(\#fig:shinyModel)모델을 개발에 사용된 설정 보기.</p>
</div>
...
<p class="caption">(\#fig:shinyPerformanceSum)설정된 임계값에 따른 성능 측정값 요약표</p>
</div>
...
<p class="caption">(\#fig:shinyPerformanceDisc)모델의 전체적인 판별도를 평가하기 위한 ROC와 정밀도-검출률 도표.</p>
</div>
...
<p class="caption">(\#fig:shinyPerformanceDist)결과가 생긴 군과 생기지 않은 군에서 예측 위험 분포. 이 도표들간에 중복이 심할 수록 판별력은 나빠진다.</p>
</div>
...
<p class="caption">(\#fig:shinyPerformanceCal)위험 층화 보정 및 인구통계학적 보정</p>
</div>
...
<p class="caption">(\#fig:shinyModelPlots)모델 요약 도표. 각 점은 모델에 포함된 변수에 해당한다.</p>
</div>
