...
평가를 위해서는 모델을 개발하는 데 사용된 것과 다른 데이터 세트를 사용해야 한다. 그렇지 않으면 과적합 (\@ref(modelFitting)절 참조) 되거나 새로운 환자에게 잘 맞지 않는 모델을 만들 위험성이 있다.
```
...
- “**표본추출 실행**”: 표본추출을 실행할지 말지 선택할 수 있다 (기본값 = “아니오 [NO]”). 만약 “예 [yes] ”라고 설정하면, 또 다른 옵션이 나타날 것이다: “**몇 명의 환자를 부분집합으로 사용할 것인가?**”. 여기서 표본 크기는 결정될 수 있다. 표본추출은 대규모 모집단 (예로 1,000만 환자)의 모델이 환자의 표본을 가지고 모델을 테스트하고 작성함으로써 예측 가능한지 여부를 결정하는 효율적인 수단이 될 수 있다. 예를 들어, 그 표본에서 AUC가 0.5에 가까우면, 그 모델은 쓸모가 없다. 
...
- “**공변량 표준화**”: 여기서 공변량을 표준화할 것인지 선택할 수 있다 (기본값 = “예 [yse]”). 공변량의 표준화는 보통 LASSO 모델의 성공적인 요소로 필요로 한다.
...
- "**검증/학습 세트를 분할하는 방법 지정:**" 학습/검증 데이터를 사람 (관심 결과에 따라 분류됨) 별로 구분할지, 시간 (모델을 학습하기 위해 이전 데이터, 나중에 모델을 평가하기 위한 이전 데이터) 별로 구분할지를 선택한다.
...
연구를 내보내려면 “유틸리티 [Utilities] ” 아래의 “내보내기 [Export] ”탭을 클릭한다. ATLAS는 연구를 실행할 때 필요한 연구 이름, 코호트 정의, 선택된 모델, 공변량, 설정과 같은 모든 데이터를 포함하는 파일에 직접 복사하여 붙여넣을 수 있는 JSON을 생성할 것이다. 
...
ATLAS 대신 R에서 코드를 직접 작성해서 연구 디자인을 할 수도 있다. [PatientLevelPrediction](https://ohdsi.github.io/PatientLevelPrediction/) 패키지에서 제공하는 함수를 사용할 수 있다. PLP 패키지는 OMOP CDM으로 변환된 데이터로부터 데이터 추출, 모델 구축 및 모델 평가를 가능하게 한다.
...
우선 R을 서버와 연결해야 한다. [`PatientLevelPrediction`](https://ohdsi.github.io/PatientLevelPrediction/)은 다양한 데이터베이스 관리 시스템(DBMS) 연결에 필요한 구체적인 설정을 위한 `createConnectionDetails`. Type `?createConnectionDetails` 라고 불리는 기능을 제공하는 [`DatabaseConnector`](https://ohdsi.github.io/DatabaseConnector/) 패키지를 사용한다. 예를 들어 이 코드를 사용하여 PostgreSQL 데이터베이스에 연결할 수 있다.
...
데이터를 추출의 마지막 단계는 `getPlpData` 함수를 실행하고 코호트가 저장되는 데이터베이스 스키마, 코호트와 결과를 위한 코호트 정의 ID, 해당 사람이 데이터에 포함되도록 관찰되어야 하는 코호트 기준 날짜 이전의 최소 기저 관찰 기간인 휴약기 washout period와 같은 세부사항들을 입력하는 것이고 마지막으로 이전에 생성된 공변량 설정을 입력하는 것이다.
...
[PatientLevelPrediction](https://ohdsi.github.io/PatientLevelPrediction/) 매뉴얼을 보면 `getPlpData` 기능에 사용할 수 있는 많은 변수 설정이 설명되어 있다.

`plpData` 오브젝트 만드는데는 상당한 계산시간이 필요하므로 다음에 하도록 하자. `plpData` 는 `ff` 를 쓰기 때문에 통상적인 R의 저장기능을 쓸 수 없다. 대신에 우리는 `savePlpData` function을 쓰도록 하자.

```{r tidy=FALSE,eval=FALSE}
savePlpData(plpData, "angio_in_ace_data")
```

나중에 저장한 내용을 불러오려면 `loadPlpData()` 기능을 쓰면 된다.

### 추가 포함 기준

최종 연구 모집단은 이전에 정의된 2개의 코호트에서 추가적인 제약조건을 적용하여 얻어진다. 예를 들어 최소 위험 노출 기간을 적용할 수 있으며 (`requireTimeAtRisk, minTimeAtRisk`), 이것이 결과를 가진 환자에게도 적용되는지 여부를 지정할 수 있다 (`includeAllOutcomes`). 또한 여기서 대상 코호트 시작에 관련된 위험 기간(risk window)의 시작과 끝을 지정한다. 예를 들어 위험 코호트가 시작된 후 30일부터를 위험 기간의 시작으로 하고 1년 후 종료하려면 `riskWindowStart = 30`, `riskWindowEnd = 365`로 설정할 수 있다. 어떤 경우는 위험 노출 기간이 코호트 종료일에 시작해야 하는 경우가 있다. `addExposureToStart = Ture` 로 설정하면 코호트 (노출) 시간을 시작일에 추가할 수 있다.
...
알고리즘 설정 기능에서 사용자는 각 하이퍼파라미터에 대한 적합한 값의 목록을 지정할 수 있다. 하이퍼파라미터에서 가능한 모든 조합은 학습 세트에 교차 검증을 사용하는 이른바 그리드서치에 포함된다. 만일 사용자가 어떤 값도 지정하지 않으면 기본값이 사용된다.

예를 들어 점진적 부스팅 머신에 다음 설정을 사용하는 경우: `ntrees = c(100,200), maxDepth = 4` 그리드서치는 점진적 부스팅 머신 알고리즘을 다른 하이퍼파라미터의 기본 설정과 조합하여 `ntrees = 100`과 `maxDepth = 4`로 설정하여 적용하고, 또한 `ntrees = 200`과 `maxDepth = 4`로도 조합하여 각각 적용할 것이다. 최고의 교차 검증 실행을 이끄는 하이퍼파라미터가 최종 모델으로 선택될 것이다. 우리 예제에서는 여러 하이퍼파라미터값을 조합하여 점진적 부스팅 머신을 만들기로 하였다: 
...
`runPIP` 함수는 모집단 `plpData`와 설정한 세팅값을 이용하여 모델을 훈련하고 평가한다. `testSplit` 과 `testFraction` 매개변수를 이용해서 사람 기준 또는 시간 기준으로 훈련데이터와 평가데이터를 75%-25%로 설정하여 환자 수준 예측 파이프라인을 실행할 수 있다:
...
이상의 설정으로 PLP패키지는 R의 xgboost 패키지를 사용하여 데이터의 75%로 점진적 부스팅 머신 모델을 만들고, 나머지 25%의 데이터로 모델을 평가한다. 실행 후 반환되는 데이터에는 모델과 성능 등에 대한 정보가 포함되어 있다.

`runPIP` 함수에는 기본적으로 `TRUE`로 설정된 `plpData`, `plpResults`, `plpplots`, `evaluation` 등을 저장할 수 있는 몇 가지 매개변수가 있다.
...
전체 결과물을 다음 명령어로 저장할 수 있다:

```{r tidy=FALSE,eval=FALSE}
savePlpResult(gbmResults, location = "gbmResults")
```

다음 명령어로 전체 결과물을 불러들일 수 있다:
...
연구를 실행하면 `runPLP`함수는 학습된 모델과 각 학습/테스트 세트에서의 모델 성능을 반환해준다. `viewPLP(runPLP = gbmResults)`를 실행하여 결과를 대화형으로 볼 수 있다. 이것은 프레임워크가 생성한 모든 성능 측정값과 대화형 도표를 볼 수 있는 Shiny 앱을 열 것이다 (Shiny 어플리케이션 섹션의 그림 \@ref(fig:shinySummary) 참조). 

모든 평가 도표를 폴더에 저장하려면 다음 코드를 실행하면 된다:
...
항상 외적 타당도를 수행하는 것을 권장한다. 즉 가능한 많은 새로운 데이터에 최종모델을 적용하고 성능을 평가해야 한다. 여기서 이미 다른 데이터베이스에서 데이터 추출이 수행되어 `newData` 폴더에 저장되었다고 가정한다. 이전에 만든 모델을 `model` 폴더로부터 로딩한다.
...
외부 검증 및 필요한 데이터 추출을 보다 쉽게 하기 위해 PLP 패키지는 `externalValidatePLP` 라는 함수를 제공한다. `result <- runPlp(...)`을 실행했다고 가정했을 때 모델에 필요한 데이터를 추출하여 새 데이터에 대해 평가할 수 있다. 검증 코호트가 ID 1과 2가 있는 테이블 `mainschema.dob.cohort` 과 CMD 데이터가 스키마 `cdmschema.dob` 라고 가정한다: 
...
모델을 검증할 데이터베이스가 여러 개 있는 경우 다음과 같이 실행 할 수 있다:
...
`viewPlp` 함수를 사용하면 예측 모델의 성능을 탐색하기 가장 쉽다. 이 함수는 **입력 결과 객체**가 필요하다. R에서 모델을 개발하는 경우 입력을 위해 runPLp 결과를 사용할 수 있다. 연구패키지로 만들어진 ATLAS에서 사용한다면 모델 중 하나를 로딩해야 한다 (이 예제에서는 Analysis_1을 로딩할 것이다): \index{model viewer app}
...
Shiny 앱은 그림 \@ref(fig:shinySummary)에서 볼 수 있듯이 테스트와 트레인 세트에 있는 실행 메트릭스의 요약본을 함께 보여준다. 이 결과는 트레인 세트에 있는 AUC가 0.78이고 테스트 세트에서는 0.74까지 떨어지는 것을 보여준다. 테스트 세트 AUC가 좀 더 정확한 측정이다. 전반적으로 이 모델은 ACE 억제제를 처음 사용하는 사용자에서 결과를 예측할 수 있을 것처럼 보이지만 트레인세트가 테스트 세트보다 실행력이 더 높기 때문에 다소 과적합 되었다. ROC 도표는 그림 \@ref(fig:shinyROC)에 제시되어 있다.
...
그림 \@ref(fig:shinyCal)에 있는 모델 적합 도표는 점들이 대각선 주위에 있을 때 일반적으로 관찰된 위험이 예측된 위험과 일치함을 보여준다. 그러나 그림 \@ref(fig:shinyDemo)에 있는 인구통계학적 그래프는 곡선 (예측위험) 이 40세 미만의 적색선 (관측위험) 모델과 다르기 때문에 모델이 젊은 환자에 대해 잘 보정되지 않았음을 보여준다. 이것은 대상 모집단(target population)에서 40대 미만을 제거해야 할 필요가 있다는 것을 알 수 있다 (젊은 환자들의 관찰된 위험이 거의 0이므로).
...
- 모델에 관한 기본 정보 (예를 들어, 데이터베이스 정보, 분류 유형, 위험에 노출된 시간 설정, 대상 모집단, 결과 명)
- 대상 모집단 수와 결과 발생률
- 예측 행렬(discrimination metrics): AUC, AUPRC

표 왼쪽에는 필터 옵션이 있는데 여기서는 초점을 맞출 개발/검증 데이터베이스, 모델 유형, 관심 있는 위험에 노출된 시간 설정 그리고/또는 관심 코호트가 있다. 예를 들어, 대상 모집단 “고혈압의 일차 단일 요법으로 ACE 억제제의 신규 사용자[New users of ACE inhibitors as first line mono-therapy for hypertension”에 해당하는 모델을 선택하려면 *대상 코호트[Target Cohort]* 옵션을 선택하면 된다.
...
모델의 전체적인 예측력을 보려면 “Discrimination” 탭을 클릭하면 ROC 도표, 정밀도-검출률(precision-recall) 도표, 분포 도표를 볼 수 있다. 그림의 선은 선택한 임계값 포인트에 해당한다. 그림 \@ref(fig:shinyPerformanceDisc)는 ROC와 정밀도-검출률 도표를 보여준다. ROC 도표는 모델이 연내에 결과를 얻을 사람과 그렇지 않은 사람을 구별할 수 있음을 보여준다. 그러나 결과 발생률이 낮다는 것은 거짓 양성률이 높다는 것을 의미하기 때문에 정밀도-재현율 도표를 보면 그 실행이 덜 인상적으로 보인다.
...
마지막으로 “Calibration” 탭을 클릭하여 모델의 보정(calibration)을 검사할 수 있다. 그림 \@ref(fig:shinyPerformanceCal)은 적합력 그림과 인구통계학적 적합력을 보여준다.
...
그림 \@ref(fig:shinyModelTable)의 표에는 공변량과 공변량으로 사용될 수 있는 모든 변수의 값 (일반 선형 모델을 사용할 경우 계수, 그렇지 않을 경우 변수 중요도), 그리고 결과 평균 (결과가 있는 사람들의 평균), 비결과 평균 (결과가 없는 사람들의 평균) 이 나타나 있다.
...
```{block2, type='rmdimportant'}
예측 모델은 인과관계를 판단하는 모델이 아니며 예측 변수들을 결과의 원인으로 오인해서는 안 된다. 그림 \@ref(fig:shinyModelTable)의 변수를 수정해도 결과의 위험에 영향을 미친다는 보장은 없다.
```
...
- 모델 개발을 위한 최고의 머신 알고리즘 선택은 경험적인 문제이다. 즉 당면한 문제와 데이터에 의해 결정된다.

- PatientLevelPrediction 패키지는 OMOP-CDM의 데이터를 사용하여 예측 모델을 개발하고 검증하기 위한 사례를 제공한다.

- 모델 예측 및 모델 성능 측도는 인터랙티브 대시보드(interactive dashboard)로 수행된다.
