...
임상 예측 모형을 설명하는 출판물의 수가 지난 10년 동안 많이 증가했다. 현재 사용되는 대부분 모형은 소규모 데이터 집합을 사용하여 추정되며, 소규모 환자 특성만 고려한다. 이처럼 소규모 표본으로인한 낮은 통계적 검정력으로 인해 데이터 분석가는 엄격한 가정하게 모델링을 수행하게 된다. 몇 가지 안되는 환자 특성을 고를 경우에는 손에 잡히는 전문 지식에 많이 의존할 수 밖에 없다. 이는 환자들이 풍부한 디지털 흔적을 생성하고 있는 현대 의학의 현실 과 크게 대조되며, 그러한 풍부한 디지털 흔적은 어떤 의료 전문가도 충분히 소화하여 내 것으로 만들 수가 없다. 현재 의료는 엄청난 양의 환자 개인별 정보를 생성하여 전자의무기록 (Electronic Health Records, EHR)에 저장하고 있다. 여기에는 진단, 약물치료, 검사 결과와 같은 정형화된 데이터와 임상 자유진술문 clinical narratives에 포함된 비정형화된 데이터가 포함되어 있다. 환자의 완전한 전자의무기록안에 저장된 대량의 데이터를 이용한다면, 예측모델의 정확성을 얼마나 올릴 수 있을지 상상도 할 수 없다.. \index{prediction model}

대규모 데이터 셋을 이용하여 기계학습의 발전으로 이런 대규모 데이터를 이용한 환자 수준 예측의 발전으로 이러한 유형의 데이터를 환자-수준 예측모형 개발에 적용하고자 하는 관심이 높아졌다. 그러나 환자-수준 예측을 수행한 출판물들을 들여다 보면, 많은 연구자들이 예측 모형 개발 지침을 따르지 않고 있어서 광범위한 외적 타당도 수행을 하지 못하거나, 또는 그 모형을 재현하고 외적 타당도를 검증하기 위해 필요한 충분한 세부 사항을 제공하지 않아서 다른 독립적인 연구자들이 그 모형을 사용하기 어렵게 하고 있다. 그것은 모형의 예측 성능을 공정하게 평가하기 어렵게 하고, 임상 실무에서 그 모형이 적절하게 사용될 가능성을 낮추고 있다. 표준화를 개선할 수 있는, 예측 모형을 개발하고 보고하는 모범 사례에 대한 지침을 자세히 설명하는 여러 논문이 출판되었다. 예를 들어 '개인별 예후/진단 예측을 위한 다변량 예측모델 투명하게 보고하기 (Transparent Reporting of a multivariable prediction model for Individual Prognosis Or Diagnosis, TRIPOD)'란 선언문[^tripodUrl] 은 예측 모형 개발 및 타당도 보고에 대한 명확한 권장 사항을 제공하고 있으며, 투명성과 관련된 일부 우려를 설명하고 있다. \index{machine learning} \index{TRIPOD}
...
표 \@ref(tab:plpDesign)에서 볼 수 있듯이 예측 문제를 밝히려면 대상 코호트(target cohort)의 t = 0, 결과 코호트(outcome cohort)에 의해 예측하고자 하는 결과, 그리고 위험 노출 시간(time-at-risk)을 정의해야 한다. 표준 예측 질문을 다음과 같이 정의한다: \index{target cohort} \index{outcome cohort} \index{time-at-risk}
...
“**대상 코호트에 적용되는 최소 휴약기간**” 최소 기준 기간, 환자가 지속적으로 관찰되는 코호트 시작일 이전의 최소 일수를 지정한다. 365일이 기본값이다. 가장 적은 lookback을 늘이는 것은 환자 (더 오래 관찰해야 하므로)를 보다 완벽하게 파악할 수 있지만, 최소 관찰일 수가 없는 환자를 필터링해야 할 것이다.
