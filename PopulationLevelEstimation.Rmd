...
```{r caseCrossover, fig.cap='환자-교차 연구 설계. 결과 근처의의 시간을 결과 날짜 이전의 미리 정의된 간격으로 설정된 제어 날짜와 비교한다.',echo=FALSE, out.width='90%', fig.align='center', fig.pos='h'}
...
```{r ncConceptSet, fig.cap='음성 대조군 컨셉 집합.',echo=FALSE, out.width='100%', fig.align='center'}
...
```{r covsToExclude, fig.cap='배제 컨셉을 정의하는 개념 집합.',echo=FALSE, out.width='100%', fig.align='center'}
...
```{r comparisons2, fig.cap='음성 대조군에 대한 개념 집합과 배제 컨셉을 보여주는 비교 창The comparison window showing concept sets for negative controls and concepts to exclude.',echo=FALSE, out.width='100%', fig.align='center'}
...
```{r tar, fig.cap='두 코호트에 있는 피험자의 위험 노출 기간(TAR)은 치료 시작 다음날부터 시작하여 노출 끝에서 멈춘다고 가정한다.',echo=FALSE, out.width='90%', fig.align='center'}
...
```{block2, type='rmdimportant'}
코호트 연구를 설계할 때, 황금률(golden rule)은 편향(bias)이 개입될 수 있으므로 연구 대상을 정의하기 위해 코호트 시작일 이후의 정보를 절대로 사용하지 않는 것이다. 예를 들어, 모든 피험자들에게 적어도 1년의 time-at-risk가 요구된다면, 우리는 치료를 잘 견디는 피험자들로 분석을 제한했다는 뜻이다. 따라서, 이러한 설정은 세심한 주의를 기울여 사용해야 한다.
```
...
```{r psSettings, fig.cap='성향 점수 조절 설정.',echo=FALSE, out.width='100%', fig.align='center'}
...
```{r outcomeModelSettings, fig.cap='결과 모델 설정.',echo=FALSE, out.width='100%', fig.align='center'}
...
```{r attrition, fig.cap='Attrition diagram. 맨 위에 표시된 계수는 우리의 목표와 대조군 코호트 정의를 충족하는 계수들이다. 하단의 계수는 결과 모델에 입력되는 계수로서, 이 경우는 Cox 회귀 분석이다.',echo=FALSE, out.width='70%', fig.align='center'}
...
- Population-level estimation는 관찰형 데이터의 인과 관계를 추론하는 것을 목적으로 한다.
