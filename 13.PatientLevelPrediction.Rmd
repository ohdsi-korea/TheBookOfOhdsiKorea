...
`plpData` 객체를 생성하는 데 상당한 시간이 걸릴 수도 있다. 그리고 향후 세션을 위해 이를 저장하는 것은 아마 좋은 생각일 것이다. 왜냐하면 `plpData`는 `ff`를 사용하고, R의 기본 저장 기능을 사용할 수 없기 때문이다. 대신에, `savePlpData` 함수를 사용해야 한다:

```{r tidy=FALSE,eval=FALSE}
savePlpData(plpData, "angio_in_ace_data")
```

향후 세션에서 데이터를 불러오기 위해서는 `loadPlpData()` 함수를 사용할 수 있다. 
