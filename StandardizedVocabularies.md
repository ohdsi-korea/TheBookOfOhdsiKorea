...
<div class="figure" style="text-align: center">
<img src="images/StandardizedVocabularies/domains.png" alt="procedure vocabularies인 CPT4와 HCPCS의 도메인 할당 비율. 직관적으론, 이러한 vocabularies는 한 도메인의 코드와 concepts를 포함하고 있어야 하지만, 실제로는 도메인들이 혼재되어있다." width="70%" />
<p class="caption">(\#fig:domains)procedure vocabularies인 CPT4와 HCPCS의 도메인 할당 비율. 직관적으론, 이러한 vocabularies는 한 도메인의 코드와 concepts를 포함하고 있어야 하지만, 실제로는 도메인들이 혼재되어있다.</p>
</div>
...
<div class="figure" style="text-align: center">
<img src="images/StandardizedVocabularies/hierarchy.png" alt="Standard, non-standard source 및 분류 개념 및 condition 도메인에서의 계층적 관계.SNOMED는 대부분 standard condition concepts (일부 ICDO3에서 파생된 종양학 관련 개념)에 사용되고, MedDRA 개념은 계층 분류 개념에 사용되는데, 다른 모든 vocabularies는 비표준 개념이나 소스 개념이 포함되어 있어서 계층 구조에는 포함하지 않는다." width="100%" />
...
<p class="caption">(\#fig:conceptValue)source concept과 Standard Concepts 사이의 One-to-many mapping. 선 조합 (Pre-coordinated) 개념은 두 가지 개념으로 나뉘는데, 하나는 속성(여기서는 history of clinical finding)과 다른 하나는 값(peptic ulcer)이다. "Maps to" 관계는 measurement 또는 observation 도메인에 매핑되는 반면, "Maps to value" 개념에는 도메인 제한이 없다.</p>
</div>
