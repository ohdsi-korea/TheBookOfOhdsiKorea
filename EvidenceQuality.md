...
\BeginKnitrBlock{rmdsummary}<div class="rmdsummary">- 우리가 생성한 근거는 *반복 가능성(repeatable)*, *재현 가능성(reproducible)*, *복제 가능성(replicable)*, *일반화 가능성(generalizable)*, *완건성(robust)*을 갖추어야 하며 *보정된(calibrated)* 결과여야 한다.
