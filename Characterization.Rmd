...
```{r atlasCharacterizationResultsExplore, fig.cap='특성 분석 결과 - 하나의 concept에 대하여 자세한 내용이 들어있는 새로운 창.',echo=FALSE, out.width='100%', fig.align='center'}
...
```{block2, type='rmdimportant'}
`aggregated = TRUE`로 바꾸면 위에 표시된 모든 사례에 대하여 FeatureExtraction로 하여금 모든 요약 통계치를 표시하도록 한다. 이 지표를 제외하면 코호트 내의 각의 사람에 대한 변수 값이 계산 될 것이다.
```
...
```{r atlasIncidenceCohortSelection, fig.cap='발생률 분석에 사용할 대상 코호트와 outcome코호트의 정의.',echo=FALSE, out.width='100%', fig.align='center'}
...
```{r atlasIncidenceStratifyFemale, fig.cap='여성에 대한 틍화를 정의한 발생률.',echo=FALSE, out.width='100%', fig.align='center'}
...
celecoxib 사용자의 질병 자연 경과에 대해 더 알고 싶다. celecoxib의 새로운 사용자들에 대한 간단한 코호트를 만드십시오. 이 때, 365일의 washout 기간을 주십시오 (어떻게 해야 하는지 자세히 알고 싶다면 \@ref(Cohorts)장을 참고십시오). 그리고 ATLAS에서 이 코호트의 characterization을 생성하고, 동병상병 질환과 약물 노출을 찾아보십시오.
